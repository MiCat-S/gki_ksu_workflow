...
REVIEWED_INPUTS = {
    "6679d544a0e87c4d690f20277bfb4b539184c27f941c474b1b2a7957f61160ec",
    "8d844f80b6155c1a2b1af552dfb0b95f8e185e57873a494d9a64531e5c9a9187",
    "cbc17ec86586163efca61098e033a1f524b4d1566e57d322f7860af5b8275637",
    "d8aadfd1bb10fd4b8b69f69fbe69390d7e8fc2d950b3d589b1ea13dacb202cbf",
    "2a5423874b090c348e356bcd81ac07be70b38c107739ef07d46c3d663bbcb0f9",
    "0bdccfd7b2f401415e141220c3a6c6aa347a94dc0bd4a44a93666dff88993c90",
    "099462148b57ddfb771af53e0412df1e827933732d91908655c06122fd75fab8",
}
...
def read_patch(filename):
    try:
        with open(filename, "rb") as f:
            content = f.read()
        if hashlib.sha256(content).hexdigest() not in REVIEWED_INPUTS:
            raise ValueError("Unreviewed SUSFS patch; refusing heuristic conversion")
...
def process_normal_file(body, target):
    ...
    while i < len(body):
        ...
        if is_ksu_susfs_if(clean_line):
            ...
            if endif_idx == -1:
                raise ValueError(f"Unclosed CONFIG_KSU_SUSFS block in {target}")
...
def process_patch(patch):
    ...
    if remaining_hooks:
        raise ValueError(f"Inline KSU hooks remain in {target}: {remaining_hooks}")
