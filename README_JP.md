...
> [!TIP]
> 通常のビルドは選択したバリアントとサブレベルごとにカーネルを生成します。6.12 の既定サブレベルは `23` です。`validate-stack` は代表的なカーネル 54 件、Android 10 件、対応 SUSFS モジュールを検証し、Release は作成しません。
...
| **LZ4/Zstd zram バックエンド** | オプションでカーネルの LZ4 と Zstd 実装を公式 LZ4 1.10.0 および Zstandard 1.5.7 に更新し、kernel 6.12 の zram バックエンドを有効化します。起動不能が確認されているため、6.12.81 では強制的に無効化されます。 |
